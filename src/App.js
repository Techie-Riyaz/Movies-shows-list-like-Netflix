...
let selectedGenre=""
let genres=[]
data.movies.map(item=>item.genres.map(genre=>genres.push(genre)));
genres=[...new Set(genres)];

let obj={}
let resGenre=genres.map(m=>obj[m]=data.movies.filter(f=>f.genres.includes(m)));
...
function App() {
  ...
  let genreList=genre.length?Object.keys(obj).filter(f=>f.toLowerCase().includes(genre)):Object.keys(obj)
  return (
    <div className="root">
       
        <div className="App-container">
        <span class="app-title">WOOKIE MOVIES</span>
        <div className='search' style={{width:'100%',textAlign:'right'}}>
        <Search options={Object.keys(obj)} handleSearch={handleSearch}/>
      </div>
     ...
     {genreList.map(f=>{
        return (
          <div>
            <div className='text-left' ><h2>{f}</h2></div>
            {/* style={{boxShadow: 'rgba(0, 0, 0, 0.35) 0px 5px 15px'}} */}
          <List data={Object.values(obj[f])}/>
          </div>
        )
      })}
     </div>
